...
from Teclado import Teclado_FIXED, TecladoVirtualA
...
sys.path.append('/usr/lib/enigma2/python/Plugins/Extensions/archivostv/server')
from archivostvserver import scrapertools, httptools
sys.path.append('/usr/lib/enigma2/python/Plugins/Extensions/archivostv/cosas')
from megadede import Recap, NavegarPeliculas, Enlaces, Enlaces1, CompruebaLogin
from mediaitem import *
from proloader import *
sys.path.append('/usr/lib/enigma2/python/Plugins/Extensions/archivostv/Modulos')
from ArchivostvModulos import html_parser_moduls
...
class iptv_streamse():
    ...
    def read_config(self):
        try:
            tree = ElementTree()
            xml = tree.parse('/usr/lib/enigma2/python/Plugins/Extensions/archivostv/cosas/ArchivostvConfig.xml')
    ...
    def get_list(self, url = None, send_mac = True):
        ...
        try:
            if url == 'playlist_history':
                ...
                tree = ElementTree()
                xml = tree.parse('/usr/lib/enigma2/python/Plugins/Extensions/archivostv/cosas/megadede.xml')
            elif url.find('hdfull.comcom') != -1:
                tree = ElementTree()
                xml = tree.parse('/usr/lib/enigma2/python/Plugins/Extensions/archivostv/cosas/hdfull.xml')
            elif url == None or url == '':
                tree = ElementTree()
                xml = tree.parse('/usr/lib/enigma2/python/Plugins/Extensions/archivostv/cosas/archivostv.xml')
            elif url == 'favorites':
                tree = ElementTree()
                if self.server_oki:
                    xml = tree.parse('/usr/lib/enigma2/python/Plugins/Extensions/archivostv/cosas/ArchivosFavorites.xml')
    ...
    def _request(self, url, mac = None):
        ...
        try:
            req = urllib2.Request(url, None, {'User-agent': 'Mozilla/5.0 archivostv 0.1',
             'Connection': 'Close'})
    ...
    def write_favorites(self):
        ...
        if self.server_oki:
            try:
                fileObj = open('/usr/lib/enigma2/python/Plugins/Extensions/archivostv/cosas/ArchivostvFavorites.xml', 'w')
                ...
                if entry[3] != '':
                    if entry[3].find('http') == -1:
                        ...
                        if STREAMS.img_loader == False:
                            self.picfile = '%s/archivostv_tmp_pic.jpg' % STREAMS.images_tmp_path
                        else:
                            m = hashlib.md5()
                            m.update(entry[3])
...
class artvPlaylist(Screen):
    ...
    def search(self):
        ...
        if STREAMS.search_text != '':
            self.session.openWithCallback(self.searchResult, Teclado_FIXED, title=_('Enter your search term(s)'), text='')
        ...
        try:
            if STREAMS.video_status:
                ...
                if STREAMS.play_iptv == True:
                    self.session.open(artvIPTVplayer)
                elif STREAMS.play_vod == True:
                    self.session.open(artvplayer)
        except Exception as ex:
            print ex
    ...
    def ok_checked(self):
        try:
            ...
            if tipo.find('megadede') != -1:
                ...
                if tipo.find('megadedeunidad') != -1 and nombre.find('La Unidad') != -1 or tipo.find('megadedeunidad') != -1 and nombre.find('Pagina Siguiente') != -1:
                    from megadede import launidad
                    LanzaPelis = launidad(self, selected_channel[1], selected_channel[10])
                    STREAMS.get_list(LanzaPelis)
                    self.update_channellist()
                    return   

                if tipo.find('megadedeListas') != -1 and nombre.find('Navegar en Listas') != -1 or tipo.find('megadedeListas') != -1 and nombre.find('Pagina Siguiente') != -1:
                    from megadede import NavegarListas
                    LanzaPelis = NavegarListas(self, selected_channel[1], selected_channel[10])
                    STREAMS.get_list(LanzaPelis)
                    self.update_channellist()
                    return                     
            ...
            if playlist_url != None:
                ...
                STREAMS.play_vod = False
                self.session.openWithCallback(self.check_standby, artvplayer)
        except Exception as ex:
            print ex
            print 'ok_checked'
    ...
    def play_iptv(self):
        if self.banned == True:
            ...
            if stream_url != None:
                STREAMS.video_status = True
                STREAMS.play_iptv = False
                self.session.openWithCallback(self.check_standby, artvIPTVplayer)
        return
...
class artvplayer(Screen, InfoBarBase, IPTVInfoBarShowHide, InfoBarSeek, InfoBarAudioSelection, InfoBarSubtitleSupport):
    STATE_IDLE = 0
    STATE_PLAYING = 1
    STATE_PAUSED = 2
    ...
    def EnlaceLoad(self, enlace):
        video_urls = []
        header = {'Referer':enlace}

        data = httptools.downloadpage(enlace, headers=header, cookies=False, bypass_cloudflare=False).data
        subtitle = scrapertools.find_single_match(data, '<track kind="captions" src="([^"]+)" srclang="es"')
        
        buscaBorrado = re.findall(r'It maybe got (.*?) by the owner', data)
        
        if buscaBorrado == []:
            pass
        else:
            Mensaje = "\n\nEl archivo esta borrado, prueba otro enlace."
            mybox = self.session.open(MessageBox, Mensaje, MessageBox.TYPE_INFO, timeout=10)
            mybox.setTitle(_("ERROR OPENLOAD"))
            return 0
        ...
        try:
            ...
            print "Este es el Error: " + str(ERRR)
            import traceback
            print(traceback.format_exc())
            # Falla el modo, se utiliza la api aunque en horas punta no funciona
            videourl, extension = self.get_link_api(enlace)
    ...
    def parse_url(self):
        ...
        try:
            ...
            if url.find('openload') != -1 or url.find('oload') != -1:
                EnlaceReal = self.EnlaceLoad(url)
                print "Este es el enlace real: " + EnlaceReal
                return EnlaceReal
...
def Plugins(**kwargs):
    return [PluginDescriptor(name='archivostv', description='Home.Media.Stream', where=PluginDescriptor.WHERE_MENU, fnc=menu), PluginDescriptor(name='archivostv', description='Home.Media.Stream', where=PluginDescriptor.WHERE_PLUGINMENU, fnc=Start_iptv_palyer, icon='plugin.png')]
    ...
    def keyOK(self):
        ...
        if current:
            if current[0] == 'movie':
                ...
                self.session.open(artvplayer, sref)
            else:
                job = current[0]
...
class AjustesPluginArchivosTV(Screen):
    ...
    def iniciarPlugin(self):
        self.session.open(artvPlaylist)
